...
@section('content')
                    ...
                    <div class="glass-panel p-4 sticky-top shadow-md border-0" style="top: 110px;">
                        ...
                        <div class="mb-5">
                            ...
                            <div class="form-check custom-check mb-3">
                                <input class="form-check-input filter-input" type="radio" name="category" value="" id="catAll" {{ !request('category') ? 'checked' : '' }}>
                                <label class="form-check-label small fw-500" for="catAll">Tất cả danh mục</label>
                            </div>
                            @foreach($categories as $cat)
                            <div class="form-check custom-check mb-3">
                                <input class="form-check-input filter-input" type="radio" name="category" value="{{ $cat->slug }}" id="cat{{ $cat->id }}" {{ request('category') == $cat->slug ? 'checked' : '' }}>
                                <label class="form-check-label small fw-500" for="cat{{ $cat->id }}">{{ $cat->name }}</label>
                            </div>
                            @endforeach
                        </div>
                        ...
                        <div class="mb-5">
                            ...
                            <select name="price_range" class="form-select border-light border-2 rounded-3 py-2 small shadow-sm filter-input">
                        ...
                        <div class="mb-5">
                            ...
                            <div class="brands-grid d-flex flex-wrap gap-2">
                                ...
                                <span class="badge {{ request('brand') == $brand ? 'bg-primary' : 'bg-light text-dark' }} py-2 px-3 border border-light-subtle rounded-3 brand-badge" 
                        ...
                        <div class="d-grid mt-4">
                            ...
                            <a href="{{ route('client.products.index') }}" class="btn btn-link small text-muted mt-2">Xóa bộ lọc</a>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-lg-9">
                    <div class="d-flex flex-wrap justify-content-between align-items-center mb-5 gap-3">
                        <div>
                            ...
                            <p class="text-muted small mb-0">Hiển thị {{ $products->firstItem() ?? 0 }} - {{ $products->lastItem() ?? 0 }} trong số {{ $products->total() }} sản phẩm</p>
                        </div>
                        <div class="d-flex gap-3 align-items-center bg-white p-2 rounded-4 shadow-sm border border-light">
                            ...
                            <select name="sort" class="form-select border-0 bg-transparent shadow-none w-auto fw-600 extra-small filter-input" style="cursor: pointer;">
                                ...
                                <div class="product-card h-100">
                                    <div class="product-img-wrapper">
                                        ...
                                        <div class="product-actions">
                                            <button type="button" class="btn btn-md btn-white rounded-circle shadow-md p-2"><i class="far fa-heart text-danger"></i></button>
    ...
    <style>
        ...
        .brand-badge { transition: all 0.2s; }
        .brand-badge:hover { background-color: #009688 !important; color: white !important; }
        .brand-badge.bg-primary { background-color: #009688 !important; }
    </style>
    ...
    <script>
        ...
        // Helper function for toast (assuming you have one, if not we'll use alert)
